#!/usr/bin/env bash
# =================================================================
# F1R3FLY CI Runner Setup Script (f1r3node / Rust)
# =================================================================
# Provisions an OCI (or any Ubuntu 22.04) instance as a GitHub Actions
# self-hosted runner with all dependencies for f1r3node Rust CI jobs.
#
# Usage:
#   ./setup-f1r3node-rust-runner.sh \
#     --repo F1R3FLY-io/f1r3node \
#     --name ci-rust-amd64 \
#     --labels self-hosted,linux,x64,f1r3fly-rust-ci,oracle-cloud \
...
Usage: $0 --repo OWNER/REPO --name NAME --labels LABELS --token TOKEN
  ...
  --name     Runner name (e.g. ci-rust-amd64)
  --labels   Comma-separated labels (e.g. self-hosted,linux,x64,f1r3fly-rust-ci,oracle-cloud)
...
echo "=== F1R3FLY CI Runner Setup (Rust) ==="
echo "  Repo:    $REPO"
...
echo "--- [3/6] Installing Python 3.10 + Poetry ---"
...
echo "--- [4/6] Creating runner user and directories ---"
...
# Install Poetry for the runner user
...
# Install Rust toolchain for the runner user
CARGO_BIN="/home/${RUNNER_USER}/.cargo/bin"
if [ ! -d "$CARGO_BIN" ]; then
    ...
    echo "Rust toolchain already installed: $(sudo -u "$RUNNER_USER" bash -c "export PATH=$CARGO_BIN:\$PATH && rustc --version")"
fi

# -- 5. GitHub Actions runner --------------------------------------
echo "--- [5/6] Installing GitHub Actions runner v${RUNNER_VERSION} ---"
...
Environment="PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/home/${RUNNER_USER}/.local/bin:/home/${RUNNER_USER}/.cargo/bin"
