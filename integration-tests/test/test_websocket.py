...
def _connect_ws(ws_url: str, events: List[dict], errors: List[str], timeout: float = 30) -> Tuple:
    ...
    def on_error(ws, error):
        msg = str(error)
        if any(expected in msg for expected in expected_disconnect_errors):
    ...
    raise AssertionError(
        f"WebSocket failed to connect to {ws_url} within {timeout}s. Errors: {errors}"
    )


def _wait_for_events(events: List[dict], required: Set[str], timeout: float = 60) -> None:
...
def _validate_block_event(event: dict) -> None:
    ...
    assert not missing, (
        f"{event_type} payload missing fields: {sorted(missing)}. Got: {sorted(payload.keys())}"
    )
    ...
    assert isinstance(payload["deploys"], list), f"{event_type} deploys should be a list"
    assert isinstance(payload["seq-num"], int), f"{event_type} seq-num should be an int"
...
def test_startup_events_validator(ws_shard: WsShardResult) -> None:
    ...
    for event in events:
        ...
        if event_type in ("approved-block-received", "entered-running-state"):
            assert event.get("schema-version") == 1, f"{event_type} wrong schema-version: {event}"
            assert "payload" in event, f"{event_type} missing payload: {event}"
            ...
            assert len(event["payload"]["block-hash"]) > 0, f"{event_type} has empty block-hash"

        elif event_type == "node-started":
            assert event.get("schema-version") == 1, f"node-started wrong schema-version: {event}"
            assert "payload" in event, f"node-started missing payload: {event}"
...
def test_startup_events_boot(ws_shard: WsShardResult) -> None:
    ...
    for event in events:
        ...
        if event_type in ("sent-unapproved-block", "sent-approved-block"):
            ...
            assert "payload" in event, f"{event_type} missing payload: {event}"
            payload = event["payload"]
            assert "block-hash" in payload, f"{event_type} missing block-hash: {payload}"
            assert "sender" in payload, f"{event_type} missing sender: {payload}"
