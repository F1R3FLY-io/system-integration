...
def _assert_all_finalized(producers, all_nodes, deploy_ids: List[str], timeouts, label: str):
    ...
    return resolve_deploy_verdicts(all_nodes, deploy_ids, timeouts.finalization * 3, label=label)
...
def _finalized_names(deploy_ids: List[str], verdicts) -> List[str]:
    ...
    return [name for name, did in zip(_PRODUCER_KEYS, deploy_ids) if did in finalized]
...
def test_overdraft_allows_exactly_one_transfer(user_shard, timeouts):
    """Two concurrent transfers from the same source each fit alone.

    Together, the transfers overdraw the source. Exactly one transfer must land.
    The source must stay nonnegative, and every node must agree on the result.
    """
    shard = user_shard
    all_nodes = shard.all_nodes
    ro = shard.node("readonly")
...
def test_concurrent_delete_and_set_same_key(user_shard, timeouts):
    ...
    try:
        ...
        for rnd in range(_CONFLICT_ROUNDS):
            ...
            ids = _deploy_on_each(shard, term_for, timeouts)
            verdicts = _assert_all_finalized(producers, all_nodes, ids, timeouts, f"del-set-{rnd}")
            ...
            settled = _await_map_settles(
                all_nodes,
                "ucc_delset",
                accept=lambda m, xa=x_allowed, ye=y_expected: m.get("y") == ye and m.get("x") in xa,
                allowed_keys={"x", "y"},
                timeout=timeouts.finalization * 3,
                label=f"del-set-{rnd}",
            )
