...
**Background load:** a `_BackgroundLoad` thread runs **the whole test** — same-vault transfers (`_BG_SRC → _BG_DST`) round-robin across the genesis producers, contending the SAME two IntegerAdd balance channels (the production merge surface) and generating netPhlo so rewards accrue. It is paused **only** for the reward case-2 idle sub-window (to prove no-accrual without traffic), then resumed.
...
3. **Reward window 1:** under bg-on, FS rewards accrue **proportionally** to stake — hard gate `Δgenesis < ΔV4 < ΔV5` (weights 1:2:3) plus positive accrual (cases 1, 5). Then **case 2**: bg paused, advance heartbeat-only (idle) blocks, assert the FS rewards total does not grow; bg resumed.
...
10. **Read sanity:** `getCoopVault` / `getInitialPosVault` decode to sane tuples.
...
## Out of scope
Slashing (covered on the dev branch), random-beacon (`commit/revealRandom`), `posVaultTransfer`, Mode-A out-of-phlo precharge failure, and reward numeric-formula exactness (behavioral assertions only).
...
- `_BackgroundLoad` (same-vault IntegerAdd contention, `pause()`/`resume()` for case 2) + `_assert_bg_load_robust`
- `PosAPI` (`bond` / `withdraw` / `read_result` / `get_bonds` / `get_rewards` / `get_pending_withdrawer` / `get_withdrawers` / `get_coop_vault` / `get_initial_pos_vault`) and `VaultAPI.get_balance` — all FS-backed via the readonly node
- `_submit_bonds` / `_submit_withdraw` / `_await_pending` / `_await_withdrawer` / `_wait_for_active` / `_await_bonds_monotone` / `_advance_lfb`
