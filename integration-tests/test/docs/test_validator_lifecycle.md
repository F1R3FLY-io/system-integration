...
**Config (module-scoped shard):** 3 genesis validators (v1/v2/v3 @ stake 100) + 3 joiners (V4=200, V5=300, V6=400) + readonly observer. All consensus knobs pinned via the shard's own `global_cli_options`: `--bond-minimum=100 --bond-maximum=1000` (makes below-min/above-max/Mode-B reachable), `--epoch-length=4`, `--quarantine-length=10` (deliberately not inherited from `conf/rust.conf`, so the poll budgets and the shard cannot drift). Distinct joiner stakes give reward weights (= bond / bond-minimum) genesis:V4:V5:V6 = 1:2:3:4, driving the reward-proportionality case.
