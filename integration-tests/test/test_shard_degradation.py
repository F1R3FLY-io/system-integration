...
def test_shard_degradation(
    ...
    logging.info(
        "Thresholds: min_rate_ratio=%.0f%%, max_desync=%d, max_stall=%d batches",
        MIN_RATE_RATIO * 100,
        MAX_DESYNC_BLOCKS,
        MAX_STALL_BATCHES,
    )
    ...
    inclusion_indices = set(range(0, TOTAL_DEPLOYS, max(1, TOTAL_DEPLOYS // INCLUSION_CHECK_COUNT)))
    ...
    for idx, (deploy_id, deploy_time, contract_type, validator_name) in enumerate(deploy_records):
        ...
        inclusion_time, block_number, finalization_time = _check_deploy_lifecycle(
            validator1_node,
            deploy_id,
            MAX_DEPLOY_INCLUSION_SECS,
            MAX_DEPLOY_FINALIZATION_SECS,
        )
        ...
        lifecycle_results.append(
            (idx, deploy_id[:16], contract_type, block_number, inclusion_time, finalization_time)
        )
    ...
    for idx, did, ctype, bnum, itime, ftime in lifecycle_results:
        ...
        logging.info(
            "  %-6d %-16s %-12s %-8s %-12s %-12s", idx + 1, did, ctype, blk_str, inc_str, fin_str
        )
    ...
    if included_but_not_finalized:
        ...
        n_pending = len(included_but_not_finalized)
        n_total = len(lifecycle_results)
        failures.append(
            f"Deploy finalization: {n_pending}/{n_total} sampled deploys "
            f"included but not finalized within their timeout: {', '.join(details)}"
        )

    if max_api_latency > MAX_API_LATENCY_SECS:
        latency_ms = max_api_latency * 1000
        threshold_ms = MAX_API_LATENCY_SECS * 1000
        failures.append(f"API latency: {latency_ms:.0f}ms exceeds {threshold_ms}ms threshold")
