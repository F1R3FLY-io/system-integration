...
def _scrape_metrics(node: Node) -> Dict[str, float]:
    ...
    try:
        resp = requests.get(
            f"http://localhost:{node.ports.http}/metrics",
            timeout=10,
        )
...
def _format_node_metrics(metrics: Dict[str, float]) -> str:
    ...
    if merge_count > 0 or replay_count > 0:
        ...
        if merge_count > 0:
            merge_ms = merge_avg * 1000
            lines.append(
                f"    parents_post_state (merge): {merge_ms:.0f}ms ({int(merge_count)} blocks)"
            )
        if replay_count > 0:
            replay_ms = replay_avg * 1000
            lines.append(
                f"    replay_block (execution): {replay_ms:.0f}ms ({int(replay_count)} blocks)"
            )
    # DAG merge breakdown
    dag_metrics = [
...
def _format_report(reports: List[PhaseReport]) -> str:
    header1 = (
        f"{'Phase':<8} | {'Deploys':>7} | {'Rate':>6} | "
        f"{'Inclusion (s)':^21} | {'Finalization (s)':^21} | {'LFB':>6}"
    )
    header2 = (
        f"{'':8} | {'':>7} | {'(d/s)':>6} | "
        f"{'p50':>6} {'p95':>6} {'p99':>6} | "
        f"{'p50':>6} {'p95':>6} {'p99':>6} | {'bl/m':>6}"
    )
    lines = [
        "",
        "LOAD TEST RESULTS",
    ...
    for report in all_reports:
        if report.node_metrics:
            logging.info(
                ...
                report.name,
                _format_node_metrics(report.node_metrics),
            )
