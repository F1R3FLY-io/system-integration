...
need a third bonded slot), uses :class:`NodeClient` to inject a crafted
malicious :class:`BlockMessage` over the TLS-encrypted P2P transport
layer, and asserts that the receiving validator records the block as
invalid and slashes the offending validator's stake to zero.
...
(threat T-37 / T-12PF **liveness arm**) is intentionally NOT
implemented, and the *reason* is stronger than "no runtime detector
exists" (which is true) — the **threat as conventionally understood
is structurally undefined in this protocol's wire semantics**.

Conventional-protocol censorship (Ethereum, Cosmos, Tendermint) is
"the proposer refuses to include other validators' transactions from
the shared mempool." In *this* protocol, **deploys are not
gossiped**. The deploy gRPC endpoint
(``casper/src/rust/casper_engine/block_admission.rs:60-94
admit_deploy``) stores deploys in the *local* node's
``KeyValueDeployStorage``; the block creator
(``casper/src/rust/blocks/proposer/block_creator.rs:52-130
prepare_user_deploys``) reads from that same local storage; the
``MultiParentCasper::deploy`` trait
(``casper/src/rust/casper_engine/traits.rs:58-63``) routes only to
``admit_deploy``. **No code path broadcasts a deploy to peers.** A
deploy submitted to v2 stays on v2 until v2 proposes it. v1 cannot
"censor" v2's deploys because v1 never has them.

This is why T-12PF is correctly classified as a *boundary assumption*
rather than a runtime offense: in shared-mempool protocols censorship
is observable and a runtime detector is sensible; in this
author-local-mempool protocol the threat reduces to "the protocol
assumes each validator gets a fair chance to propose its own block
from its own mempool" — which is exactly what proposer fairness is.
``slashing-traceability.md:161`` (finding 88 "Evidence-denial
min-cut" classified ``model_boundary`` with "No source bug
confirmed") is consistent with this: the boundary status is a
*positive* finding about the protocol's design, not a deferred
TODO.

The Rust binary correspondingly has no runtime proposer-fairness
detector: ``CasperShardConf`` has no ``proposer_fairness_*`` field,
``InvalidBlock`` has no ``CensoringProposer`` variant, and ``rg
'proposer_fairness|censor|fairness'`` over ``casper/src/rust/`` and
``node/src/rust/`` returns only two unrelated queue/stream-ordering
comments. T-12PF liveness is exercised in-tree by
``casper/tests/slashing/proposer_fairness_boundary.rs``,
``hypothesis_adversarial_scheduler.rs``,
``hypothesis_liveness_as_safety.rs``, and five other UC tests; those
satisfy the formal threat-model objective for the liveness arm.
...
pins the absence of over-eager behavioral-pattern detectors against
the deployed binary; if a future regression introduces an "inactive
validator" or "proposer dominance" slashing arm, it fails
immediately. The wire-level withholding theme is additionally
covered by :func:`test_slash_late_released_equivocation` and the
symmetric "didn't act on what you saw" arm by
...
from .common import CommandLineOptions, TestingContext
...
from .node_client import NodeClient, node_protocol_client
from .rnode import Node
from .wait import wait_for_log_match, wait_for_node_sees_block
...
# canonical identities live in ``conftest.VALIDATOR{N}_ID``; we only
...
# Standard contract used by every slash test. Inlined (rather than
# read from a path inside the container) so the test is portable across
# Scala and Rust node images and does not depend on the image bundling
# any particular example .rho file at /opt/docker/examples/.
...
def generate_block_hash() -> bytes:
    ...
    Computed once via blake2b(b'evil') so every call yields the same
    value (tests pre-compute it for the regex-match assertion).
    """
...
def rust_block_hash(block: BlockMessage) -> bytes:
    ...
    Mirrors the byte layout in ``casper/src/rust/util/proto_util.rs:380``
    of the f1r3node-rust repo exactly::
    ...
    Hashed with Blake2b at digest size 32. Forged blocks re-hashed with
    this helper pass :class:`Validate::block_hash` on the Rust node and
    reach the intended offense rather than being rejected as
    :class:`InvalidBlockHash` first.

    See the module docstring for the upstream-divergence rationale and
    the per-test smoke assertion that anchors this invariant.
    """
    buf = b''.join([
...
def _slash_shard(
    ...
    *,
...
) -> Generator[Tuple[TestingContext, CustomShard, Node, Node, NodeClient], None, None]:
    """Spawn a custom shard for a single slash test.

    Yields ``(context, shard, validator1, validator2, client)``. The
    bootstrap (ceremony master) node is on ``shard.nodes["boot"]`` and
    ``validator3`` (when ``three_validators=True``) on
    ``shard.nodes["validator3"]``. ``ftt=-1.0`` keeps fault-tolerance
    enforcement out of the way and ``heartbeat=False`` keeps the
    propose loop quiet so the test can inject crafted blocks
    deterministically.

    ``global_cli_options`` is plumbed through to
    :func:`start_custom_shard` so tests like
    :func:`test_slash_stale_evidence_rebond` can override
    ``--epoch-length`` (and reach the second epoch in 3 proposes
    instead of 10) without forking the helper.
    """
    ...
    ) as shard, node_protocol_client(
        shard.network_name, docker_client, testing_context,
    ) as client:
        yield (
            testing_context,
            shard,
            shard.nodes["validator1"],
            shard.nodes["validator2"],
            client,
        )
...
def test_slash_invalid_block_hash(
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=40900,
    ) as (context, _shard, validator1, validator2, client):
...
def test_slash_invalid_block_number(
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=40950,
    ) as (context, _shard, validator1, validator2, client):
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        blockhash = validator1.propose()
...
def test_slash_invalid_block_seq(
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=41000,
    ) as (context, _shard, validator1, validator2, client):
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        blockhash = validator1.propose()
...
def test_slash_justification_not_correct(
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41050,
    ) as (context, _shard, validator1, validator2, client):
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        blockhash = validator1.propose()
...
@_SKIP_ON_NON_LINUX
def test_slash_unauthorized_slash_deploy(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41350,
    ) as (context, shard, validator1, validator2, client):
        validator3 = shard.nodes["validator3"]

        # v1 proposes first so v2's natural propose has a parent.
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        v1_blockhash = validator1.propose()
        wait_for_node_sees_block(context, validator2, v1_blockhash)
        wait_for_node_sees_block(context, validator3, v1_blockhash)
        ...
        wait_for_node_sees_block(context, validator1, v2_blockhash)
        wait_for_node_sees_block(context, validator3, v2_blockhash)
...
@_SKIP_ON_NON_LINUX
def test_slash_references_valid_block(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41400,
    ) as (context, shard, validator1, validator2, client):
        validator3 = shard.nodes["validator3"]
...
@_SKIP_ON_NON_LINUX
def test_slash_self_regression(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=41500,
    ) as (context, _shard, validator1, validator2, client):
        # First propose: v1 builds atop genesis.
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        v1_blockhash_a = validator1.propose()
        wait_for_node_sees_block(context, validator2, v1_blockhash_a)

        # Second propose: v1's self-justification now points at block A.
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        v1_blockhash_b = validator1.propose()
        wait_for_node_sees_block(context, validator2, v1_blockhash_b)
...
@_SKIP_ON_NON_LINUX
def test_slash_invalid_bonds_cache(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=41600,
    ) as (context, _shard, validator1, validator2, client):
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        v1_blockhash = validator1.propose()
        wait_for_node_sees_block(context, validator2, v1_blockhash)
...
@_SKIP_ON_NON_LINUX
def test_slash_invalid_repeat_deploy(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=41650,
    ) as (context, _shard, validator1, validator2, client):
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        v1_blockhash = validator1.propose()
        wait_for_node_sees_block(context, validator2, v1_blockhash)
...
def test_slash_invalid_validator_approve_evil_block(
    ...
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41100,
    ) as (context, shard, validator1, validator2, client):
        validator3 = shard.nodes["validator3"]
        ...
        wait_for_node_sees_block(context, validator3, evil_block_hash.hex())
...
def test_slash_GHOST_disobeyed(
    ...
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41150,
    ) as (context, shard, validator1, validator2, client):
        validator3 = shard.nodes["validator3"]
        ...
        wait_for_node_sees_block(context, validator3, blockhash2)

        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        blockhash3 = validator1.propose()

        wait_for_node_sees_block(context, validator2, blockhash3)
        wait_for_node_sees_block(context, validator3, blockhash3)
...
def test_node_working_right_after_slashing(
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=41200,
    ) as (context, _shard, validator1, validator2, client):
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        blockhash = validator1.propose()
...
@_SKIP_ON_NON_LINUX
def test_slash_ignorable_equivocation(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    * ``check_equivocations``: ``latest_message_hash(v1) == b1.hash``
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=41300,
    ) as (context, _shard, validator1, validator2, client):
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
...
@_SKIP_ON_NON_LINUX
def test_slash_admissible_equivocation(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=41250,
    ) as (context, _shard, validator1, validator2, client):
        ...
        wait_for_log_match(context, validator2, re.compile(
...
@_SKIP_ON_NON_LINUX
def test_slash_neglected_equivocation(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    * If for some reason it doesn't → :class:`NeglectedInvalidBlock`
    ...
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41450,
    ) as (context, shard, validator1, validator2, client):
        validator3 = shard.nodes["validator3"]
        ...
        b1_hash = validator1.propose()
        wait_for_node_sees_block(context, validator2, b1_hash)
        wait_for_node_sees_block(context, validator3, b1_hash)
        ...
        for v in (validator2, validator3):
            wait_for_log_match(context, v, re.compile(
                rf"Recording invalid block {b1p_hash.hex()[:10]}\.\.\. for IgnorableEquivocation\."
            ))
        ...
        wait_for_log_match(context, validator3, re.compile(
...
@_SKIP_ON_NON_LINUX
def test_slash_late_released_equivocation(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    """
    with _slash_shard(
        testing_context, docker_client, command_line_options, port_base=41750,
    ) as (context, _shard, validator1, validator2, client):
        validator1.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_1)
        b1_hash = validator1.propose()
        wait_for_node_sees_block(context, validator2, b1_hash)
...
@_SKIP_ON_NON_LINUX
def test_slash_stale_evidence_rebond(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    * 4..5 → epoch 2
    ...
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41550,
        ...
        wait_for_node_sees_block(context, validator2, v1_a_hash)
        wait_for_node_sees_block(context, validator3, v1_a_hash)
        ...
        wait_for_node_sees_block(context, validator3, v2_hash)
        validator3.deploy_string(HELLO_CONTRACT, BONDED_VALIDATOR_KEY_3)
        v3_hash = validator3.propose()
        wait_for_node_sees_block(context, validator2, v3_hash)
        ...
        wait_for_node_sees_block(context, validator3, v2_b_hash)
...
@_SKIP_ON_NON_LINUX
def test_slash_self_correcting_block_admitted(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41700,
    ) as (context, shard, validator1, validator2, client):
        validator3 = shard.nodes["validator3"]
        ...
        wait_for_node_sees_block(context, validator2, v1_hash)
        wait_for_node_sees_block(context, validator3, v1_hash)
        ...
        wait_for_node_sees_block(context, validator3, v2_self_correcting_hash)
...
@_SKIP_ON_NON_LINUX
def test_no_false_positive_slash_on_propose_imbalance(
    testing_context: TestingContext,
    command_line_options: CommandLineOptions,
    docker_client: DockerClient,
) -> None:
    ...
    *liveness* arm (eventual slashing under fairness) cannot be
    ...
    with _slash_shard(
        testing_context, docker_client, command_line_options,
        three_validators=True, port_base=41800,
    ) as (context, shard, validator1, validator2, client):
        validator3 = shard.nodes["validator3"]
        ...
        wait_for_node_sees_block(context, validator1, v3_blockhash)
        wait_for_node_sees_block(context, validator2, v3_blockhash)
