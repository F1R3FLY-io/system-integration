...
from f1r3fly.pb.CasperMessage_pb2 import JustificationProto as Justification
...
from .rnode import Node, bootstrap_connected_peer, ready_bootstrap_with_network
from .wait import wait_for_log_match, wait_for_node_sees_block

BOOTSTRAP_NODE_KEY = PrivateKey.from_hex(
    "ff2ba092524bafdbc85fa0c7eddb2b41c69bc9bf066a4711a8a16f749199e5be"
)
BONDED_VALIDATOR_KEY_1 = PrivateKey.from_hex(
    "597623f0b50e82008d52644983699724538b4307efbe3d0b992c651ca7f860c8"
)
BONDED_VALIDATOR_KEY_2 = PrivateKey.from_hex(
    "9a32ff7b7c6e25527e0b4e5bec70596c6094e6529d56bf61cbd1ca26d3e92b10"
)
BONDED_VALIDATOR_KEY_3 = PrivateKey.from_hex(
    "af47862137d4e772f540029ae73ee01443c61288f3df9307a13d681de6ad2de4"
)
...
@contextmanager
def three_nodes_network_with_node_client(
    ...
    with (
        conftest.testing_context(
            command_line_options,
            random_generator,
            docker_client,
            bootstrap_key=BOOTSTRAP_NODE_KEY,
            peers_keys=peers_keypairs,
            validator_bonds_dict=validator_bonds_dict,
        ...
        bootstrap_connected_peer(
            ...
            name="bonded-validator-1",
            private_key=BONDED_VALIDATOR_KEY_1,
        ) as validator1,
        bootstrap_connected_peer(
            context=context,
            bootstrap=bootstrap_node,
            name="bonded-validator-2",
            private_key=BONDED_VALIDATOR_KEY_2,
        ) as validator2,
...
def test_slash_invalid_block_hash(
    ...
    with three_nodes_network_with_node_client(
    ...
    ) as (context, _, validator1, validator2, client):
        ...
        record_invalid = re.compile(
            f"Recording invalid block {evil_block_hash.hex()[:10]}... for InvalidBlockHash"
        )
...
def test_slash_invalid_block_seq(
    ...
    with three_nodes_network_with_node_client(
        ...
        client.send_block(invalid_block_num_block, validator2)
        record_invalid = re.compile(
            f"Recording invalid block {invalid_block_hash.hex()[:10]}... for InvalidSequenceNumber"
        )
        wait_for_log_match(context, validator2, record_invalid)
...
def test_slash_justification_not_correct(
    ...
    with three_nodes_network_with_node_client(
        ...
        random_generator,
        docker_client,
        validator_bonds_dict=bonded_validators,
    ) as (context, _, validator1, validator2, client):
        contract = "/opt/docker/examples/tut-hello.rho"
        ...
        record_invalid = re.compile(
            f"Recording invalid block {invalid_block_hash.hex()[:10]}... for InvalidFollows"
        )
        wait_for_log_match(context, validator2, record_invalid)
...
def test_slash_invalid_validator_approve_evil_block(
    ...
    with three_nodes_network_with_node_client(
        command_line_options, random_generator, docker_client
    ) as (context, validator3, validator1, validator2, client):
        ...
        invalid_block.justifications.extend(
            [  # pylint: disable=maybe-no-member
                Justification(
                ...
                Justification(
                    ...
                    latestBlockHash=bytes.fromhex(genesis_block.blockInfo.blockHash),
                ),
                Justification(
                    validator=BOOTSTRAP_NODE_KEY.get_public_key().to_bytes(),
                    latestBlockHash=bytes.fromhex(genesis_block.blockInfo.blockHash),
                ),
            ]
        )
        ...
        block_not_slash_invalid_block.justifications.extend(
            [  # pylint: disable=maybe-no-member
                Justification(
                    validator=BONDED_VALIDATOR_KEY_1.get_public_key().to_bytes(),
        ...
        deploy_data = create_deploy_data(
            ...
            Path("../rholang/examples/tut-hello.rho").read_text(encoding="utf-8"),
            1,
            1000000,
        )
...
def test_slash_GHOST_disobeyed(
    ...
    with three_nodes_network_with_node_client(
        command_line_options, random_generator, docker_client
    ) as (context, bootstrap_node, validator1, validator2, client):
        contract = "/opt/docker/examples/tut-hello.rho"
        ...
        record_invalid = re.compile(
            f"Recording invalid block {invalid_block_hash.hex()[:10]}... for InvalidParents"
        )
        wait_for_log_match(context, validator2, record_invalid)
