...
<!-- BUILD-COMMANDS:BEGIN
     Replace with this repo's toolchain. Examples by ecosystem:

     Rust:        cargo fmt --all && cargo clippy --workspace --all-targets && cargo test
     Node/TS:     pnpm install && pnpm run typecheck && pnpm run test && pnpm run build
     Scala/sbt:   sbt clean compile test
     Python:      uv run ruff check . && uv run pytest
     Clojure:     clojure -M:test
     Mixed:       see scripts/ or DEVELOPER.md

     /harmonize will not overwrite this block once filled in.
-->
```bash
# replace with this repo's build/test commands
```
<!-- BUILD-COMMANDS:END -->
