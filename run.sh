...
case $ACTION in
    up)
        ...
        echo ""
        docker-compose --env-file .env.node -f "$COMPOSE_FILE" up -d
        echo ""
    ...
    down)
        ...
        docker-compose --env-file .env.node -f "$COMPOSE_FILE" down
    ...
    logs)
        ...
        docker-compose --env-file .env.node -f "$COMPOSE_FILE" logs -f
        ;;
    status)
        COMPOSE_FILE=$(detect_running_compose)
        if [[ -z "$COMPOSE_FILE" ]]; then
        ...
        echo ""
        docker-compose --env-file .env.node -f "$COMPOSE_FILE" ps
        ;;
    wait)
        COMPOSE_FILE=$(detect_running_compose)
        if [[ -z "$COMPOSE_FILE" ]]; then
